...
<body style="margin:0;padding:0;background:#f4f4f5;font-family:Inter,sans-serif;">
  <div style="max-width:560px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 8px rgba(0,0,0,.08);">
    <div style="background:#f97316;padding:24px 32px;">
      ...
      ${eventTitle ? `<p style="margin:4px 0 0;color:rgba(255,255,255,.85);font-size:13px;">${escapeHtml(eventTitle)}</p>` : ""}
    </div>
    <div style="padding:32px;">
      <h2 style="margin:0 0 16px;font-size:18px;color:#111;">${escapeHtml(subject)}</h2>
      <div style="color:#444;font-size:15px;line-height:1.7;white-space:pre-wrap;">${escapeHtml(body)}</div>
    </div>
    <div style="padding:16px 32px;background:#f9fafb;border-top:1px solid #e5e7eb;font-size:12px;color:#9ca3af;text-align:center;">
      ${organizerName ? `Message envoyé par <strong>${escapeHtml(organizerName)}</strong> via ` : ""}EventFlow · La plateforme événementielle africaine
    </div>
  </div>
</body>
</html>`;
